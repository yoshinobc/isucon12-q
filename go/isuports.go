...
func tenantsBillingHandler(c echo.Context) error {
	...
	ts := []TenantRow{}
	if beforeID != 0 {
		if err := adminDB.SelectContext(ctx, &ts, "SELECT * FROM tenant WHERE id < ? ORDER BY id DESC LIMIT 100", beforeID); err != nil {
			return fmt.Errorf("error Select tenant: %w", err)
		}
	} else {
		if err := adminDB.SelectContext(ctx, &ts, "SELECT * FROM tenant ORDER BY id DESC LIMIT 100"); err != nil {
			return fmt.Errorf("error Select tenant: %w", err)
		}
	}
